package com.aguiabranca.inovacao.viewmodel
...
import com.aguiabranca.inovacao.models.IdeaType
...
class AppViewModel(
        ...
        viewModelScope.launch {
            ...
            when (val result = authRepository.getCurrentUser()) {
                is AppResult.Success -> {
                    ...
                    if (result.data != null) loadHomeData(result.data.role)
                }
                is AppResult.Error -> _uiState.update { it.copy(isLoading = false, message = result.message) }
            }
        }
    }
    ...
    fun login(email: String, password: String) {
        viewModelScope.launch {
            ...
            val result = if (loginResult is AppResult.Error && loginResult.message.contains("There is no user record", ignoreCase = true)) {
                authRepository.completeFirstAccess(request)
            } else {
                loginResult
            }
            when (result) {
                is AppResult.Success -> {
                    _uiState.update { it.copy(currentUser = result.data, isLoading = false, message = "Login realizado.") }
                    loadHomeData(result.data.role)
                }
                is AppResult.Error -> _uiState.update { it.copy(isLoading = false, message = result.message) }
            }
        }
    ...
    fun loadHomeData(role: UserRole = _uiState.value.currentUser?.role ?: UserRole.OPERADOR) {
        viewModelScope.launch {
            loadStrategies()
            when (role) {
                UserRole.ADMIN_TI -> {
    ...
    fun saveProject(title: String, description: String, investment: Double, profit: Double, progress: Int) {
        viewModelScope.launch {
            handleMutation(
